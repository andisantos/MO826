...
Com a pandemia do novo coronavírus (COVID-19), o número crescente de casos e óbitos destaca a necessidade urgente de medidas efetivas de controle da doença no Brasil. O atual esquema de vacinação contra a COVID-19 do Ministério da Saúde iniciou-se com os grupos prioritários definidos como: idosos, povos indígenas, profissionais de saúde e de serviços essenciais. O presente estudo procurou compreender a potencial influência da vacinação aplicada prioritariamente a perfis em condições mais propensas a mortalidade, considerando não apenas como critério a idade do indivíduo a ser imunizado. Levou-se em consideração o contexto pandêmico vivenciado, realizando um parâmetro com o ano de 2019 como forma de buscar por relações nos dados que possam ser capazes de fornecer uma melhor análise estatística das regiões e levantar possíveis planos de vacinação que poderiam beneficiar, de maneira mais ágil, o controle da pandemia de COVID-19.

Trata-se de um problema multidimensional, onde temos os indivíduos e suas características como entrada para os modelos supervisionados e, como variável de saída, os valores indicando óbito ou cura após infecção por COVID-19.

A análise proposta foi realizada em duas etapas: primeiramente realizou-se uma análise exploratória das variáveis para verificar comportamentos em relação a veriável de resposta afim de filtrar variáveis que não possuiam poder preditivo. A seguir, passou-se para a etapa de treinamento dos modelos supervisionados e discussão dos resultados alcançados.

Como resultados alcançados, o melhor modelo obteve 66,79% de acurácia, 66,27% de especificidade e 65,21% de sensibilidade e, em um último teste com amostras selecionadas na proporção 50%/50% revelou uma boa capacidade de ordenação e diferenciação da taxa de letalidade das partes escoradas.
